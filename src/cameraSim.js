...
function _deriveDirectorialCamera(shot) {
  const action = String(
    shot?.directorial_action ||
    shot?._director_state?.camera?.directorial_action ||
    'observe'
  ).toLowerCase();

  const motivation = String(
    shot?.camera_motivation ||
    shot?._director_state?.camera?.motivation ||
    ''
  ).trim();

  const map = {
    observe: { purpose: 'observe the character and preserve readable geography', bias: 'restrained' },
    pursue: { purpose: 'pursue the subject so the audience feels momentum and pressure', bias: 'forward' },
    reveal: { purpose: 'withhold then reveal story information through controlled reframing', bias: 'revealing' },
    conceal: { purpose: 'withhold information and keep the audience inside the character’s uncertainty', bias: 'withheld' },
    trap: { purpose: 'compress the subject against the environment and make the space feel confining', bias: 'compressed' },
    distance: { purpose: 'create emotional or physical distance without breaking spatial continuity', bias: 'retreating' },
    join: { purpose: 'move with the characters so the audience enters the shared action', bias: 'immersive' },
    witness: { purpose: 'remain an observer of the event rather than explaining it', bias: 'observational' },
    follow: { purpose: 'follow the subject through space while preserving route geography', bias: 'tracking' },
    discover: { purpose: 'guide attention toward the newly discovered story fact', bias: 'discovery' },
    confront: { purpose: 'hold the relational geometry so the power struggle is visible', bias: 'confrontational' },
    transition: { purpose: 'preserve movement and spatial causality through the transition', bias: 'transitional' },
    arrive: { purpose: 'land the audience in the destination after the physical journey completes', bias: 'settling' },
  };

  const selected = map[action] || map.observe;
  return {
    action: Object.prototype.hasOwnProperty.call(map, action) ? action : 'observe',
    purpose: selected.purpose,
    movement_bias: selected.bias,
    motivation: motivation || selected.purpose,
  };
}
...
function applyCameraSimulation(script) {
  ...
  for (const scene of script.scenes) {
    ...
    for (const shot of shots) {
      if (!shot._director_state) {
        const enriched = directorState.attachDirectorState(shot, {
          scene,
          episode: script,
          previousState,
        });
        Object.assign(shot, enriched);
      }
      ...
      const directorial = _deriveDirectorialCamera(shot);
      ...
      shot._camera_sim = {
        ...
        depthLayers,
        directorial,
        promptFragment: _buildCameraSimPrompt(focalLength, movementCurve, depthLayers, directorial),
      };

      previousState = shot._director_state || previousState;
      totalShots++;
    }
  }
 ...
 */
function _buildCameraSimPrompt(focalLength, movement, depth, directorial = null) {
  const parts = [];
  ...
  if (directorial) {
    parts.push(`Directorial motivation: ${directorial.motivation}. The camera exists to ${directorial.purpose}.`);
  }
