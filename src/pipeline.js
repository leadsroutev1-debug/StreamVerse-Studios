...
const db             = require('./db');
const episodeCharacterRecovery = require('./episodeCharacterRecovery');
...
// All providers expose the same submitVideoJob/pollVideoJob interface.
// Every provider receives the authored current-shot still as its authoritative I2V input.
// For shots after the first, that still is generated from the exact previous-shot
// terminal frame plus only the canonical character references needed in the shot.
...
/**
 * Normalize provider poll results to the pipeline's URL-only contract.
 *
 * Providers should return a string URL from pollVideoJob(), but a provider
 * adapter may temporarily return a job/result object. Never let that object
 * reach Cloudinary, where implicit coercion becomes "[object Object]" and
 * produces an opaque HTTP 400.
 */
function _normalizeVideoUrl(pollResult, context = 'video job') {
  const candidate = typeof pollResult === 'string'
    ? pollResult
    : pollResult?.video_url || pollResult?.videoUrl || pollResult?.url || '';
  const url = String(candidate || '').trim();
  if (!url) {
    const receivedType = pollResult === null
      ? 'null'
      : Array.isArray(pollResult)
        ? 'array'
        : typeof pollResult;
    throw new Error(`[Pipeline] ${context} returned no usable video URL (received ${receivedType})`);
  }
  return url;
}
...
/**
 * Validate and enforce provider-aware semantic pacing on the generated episode script.
 * Agnes receives an 18s semantic temporal canvas; LTX retains its 10s ceiling.
 * Dialogue length, environmental storytelling and within-shot progression contribute to duration.
 *
 * Returns the modified script object.
 */
// Provider-owned temporal contract. Agnes is intentionally allowed to use
// the full 18-second production canvas; LTX keeps its existing 10-second ceiling.
const _AGNES_MAX_DURATION = 18.0;
const _ACTIVE_PROVIDER_MAX_DURATION = config.videoProvider === 'agnes'
  ? _AGNES_MAX_DURATION
  : Number(config.ltxMaxDuration || 10.0);
...
const _SEMANTIC_TARGET_DURATION = config.videoProvider === 'agnes' ? 18.0 : 9.25;
const _SEMANTIC_MAX_DURATION = _ACTIVE_PROVIDER_MAX_DURATION;
...
const MAX_SHOT_DURATION = _ACTIVE_PROVIDER_MAX_DURATION;

function _videoTemporalContract() {
  const maxSeconds = _ACTIVE_PROVIDER_MAX_DURATION;
  return {
    provider: config.videoProvider,
    minSeconds: _SEMANTIC_MIN_DURATION,
    targetSeconds: config.videoProvider === 'agnes' ? 18.0 : _SEMANTIC_TARGET_DURATION,
    maxSeconds,
    authoringInstruction: config.videoProvider === 'agnes'
      ? `Write every scene/shot against an 18-second cinematic temporal canvas; preserve meaningful action, performance, environmental progression and dialogue without compressing the shot to a 10-second block.`
      : `Write shots against the active provider temporal canvas capped at ${maxSeconds}s.`,
  };
}
...
function _enforcePacingRules(script) {
  ...
  for (const scene of scenes) {
    ...
    for (const shot of shots) {
      ...
      shot.duration_source = `${source}|provider_max:${_ACTIVE_PROVIDER_MAX_DURATION}s`;
      if (config.videoProvider === 'agnes' && dynDuration >= 17.5) {
        console.log(`[Pacing] Agnes S${scene.scene_number}/idx${shot.shot_index}: using near-full 18s temporal canvas (${dynDuration.toFixed(1)}s)`);
      }
      ...
      if (config.videoProvider === 'magichour') {
        ...
        const providerMinDuration = config.videoProvider === 'agnes'
          ? _SEMANTIC_MIN_DURATION
          : config.ltxMinDuration;
        const providerMaxDuration = _ACTIVE_PROVIDER_MAX_DURATION;
        shot.duration = Math.min(
          providerMaxDuration,
          Math.max(providerMinDuration, dynDuration)
        );
      }
    }
  ...
  // Opening hooks stay compelling while respecting the active provider temporal canvas.
  if (scenes.length > 0) {
 ...
 * surfaces the same authoritative URL to the Vision Director so both stages
 * reason from identical predecessor pixels.
 */
async function _resolvePreviousShotEndFrameUrl({ episodeId, prevShot }) {
  if (!episodeId || !prevShot) return null;
  ...
  try {
    const row = await db.queryOne(
      ...
      [
        episodeId,
        Number(prevShot.scene_number),
        Number(prevShot.shot_index),
      ]
    );
    ...
    console.warn(
      `[Pipeline] Previous end-frame lookup unavailable for ` +
      `S${prevShot.scene_number}/idx${prevShot.shot_index}: ${err.message}`
    );
    return null;
  }
}
  ...
  return {
    ...
    _promptCapture: typeof episodeId === 'string' && episodeId
      ? async (prompt) => {
          ...
          const providerLabel = config.videoProvider === 'agnes'
            ? 'AGNES'
            : config.videoProvider === 'magichour'
              ? 'MAGIC HOUR'
              : 'LTX';
          console.log(
            `[Pipeline] EXACT ${providerLabel} PROMPT persisted | S${sceneNumber}/idx${shotIndex} ` +
            `chars=${exact.length}`
          );
        }
      : null,
...
async function generateShot(shot, storyline, characterList, globalEpisodeNumber, onMhSubmitted, prevShot = null, sceneBgImageUrl = null, onImageGenerated = null, reuseImageUrl = null, faceLockRegistry = new Map()) {
  ...
  /*
   * Resolve the same predecessor terminal frame used by Agnes continuity.
  ...
  // Backfill a missing terminal frame from the actual predecessor clip.
  // This repairs older episodes whose shot_continuity_frames row was never
      ...
      if (episodeId) {
        const previousRow = await db.queryOne(
          `SELECT clip_url FROM shots
             ...
             AND shot_index = ?
             LIMIT 1`,
          [episodeId, Number(prevShot.scene_number), Number(prevShot.shot_index)]
        );
        if (previousRow?.clip_url) {
          const recoveredFrames = await videoGen.retrieveContinuityFrames(previousRow.clip_url);
          previousEndFrameUrl = String(recoveredFrames?.lastFrameUrl || '').trim() || null;
          if (previousEndFrameUrl) {
            console.log(`[Pipeline] Recovered missing terminal frame | S${prevShot.scene_number}/idx${prevShot.shot_index}`);
          }
        }
      }
    } catch (frameRecoveryErr) {
      console.warn(`[Pipeline] Terminal-frame recovery failed for S${prevShot.scene_number}/idx${prevShot.shot_index}: ${frameRecoveryErr.message}`);
    }
  }

  if (prevShot) {
    console.log(
      `[Pipeline] Visual continuity anchor | ` +
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      let orderedCharsForRender;
      let charRefs;
      if (continuityReanchorMode) {
        orderedCharsForRender = selectedContinuityRefs.map(x => x.character);
        charRefs = selectedContinuityRefs.map(x => x.url);
      } else {
        orderedCharsForRender = orderedChars;
        charRefs = orderedChars.map(c => {
          try {
            const multi = safeJsonParse(c.reference_image_urls, []);
            if (Array.isArray(multi) && multi.length) return multi[0]?.url || multi[0];
          } catch {}
          return c.reference_image_url || null;
        }).filter(Boolean);
      }

      const referenceUrls = continuityReanchorMode
        ? [previousEndFrameUrl, ...charRefs].slice(0, 4)
        : (sceneBgImageUrl
      ...
      if (continuityReanchorMode) {
        const selectedDescription = selectedContinuityRefs
      ...
      // about who is where.
      const characterPositions = _assignCharacterPositions(orderedCharsForRender);
      ...
      // The image buffer produced above is the ACTUAL authored still for this shot.
      // Agnes' adapter receives this exact buffer as its current-shot keyframe.
      currentShot._actual_shot_image_supplied = true;
      ...
      } else if (config.videoProvider === 'agnes') {
        // Agnes receives the current authored/re-anchored still as the SINGLE
        // authoritative I2V keyframe. Continuity was resolved upstream while
        // creating this still from the previous shot's exact terminal frame.
        const agnesPrompt = currentShot._ltxPromptOverride
          || currentShot.ltx_shot_description
          || currentShot.shot_description
          || currentShot.image_prompt
          || '';
        submitMeta = {
          duration: Math.min(_ACTIVE_PROVIDER_MAX_DURATION, Math.max(_SEMANTIC_MIN_DURATION, currentShot.duration || _ACTIVE_PROVIDER_MAX_DURATION)),
          agnesPrompt,
          videoPrompt: agnesPrompt,
          shotPacingType: currentShot.shot_pacing_type || '',
          motionParams,
          width: Number(process.env.AGNES_WIDTH || 1024),
          height: Number(process.env.AGNES_HEIGHT || 1536),
          seed: shotSeed,
          negativePrompt: process.env.AGNES_NEGATIVE_PROMPT || '',
          ...
          /*
           * Both the video provider and the Vision Director use the exact same
           * predecessor terminal frame. Agnes consumes continuityLastFrameUrl
...
async function _runPipeline() {
  ...
  if (resumeNeedsScriptWriting) {
    ...
    try {
      episodeScript = await scriptWriter.writeEpisodeScript({
        ...
        providerTemporalContract: _videoTemporalContract(),
        sceneShotWritingDurationSeconds: _ACTIVE_PROVIDER_MAX_DURATION,
  ...
  // Stamp the active provider temporal contract onto the persisted script so
  // scene-shot writing/resume has one authoritative duration contract.
  episodeScript.provider_temporal_contract = _videoTemporalContract();
  episodeScript.scene_shot_writing_duration_seconds = _ACTIVE_PROVIDER_MAX_DURATION;
  ...
  await episodeCharacterRecovery.reconcileMissingEpisodeCharacters({
    storyline,
    episodeScript,
    characterList,
  ...
  // ── Enforce provider-aware semantic pacing ─────────────────────────────
  ...
  if (shouldPostProcessScript) {
    ...
    // ── Canonical Director State: blocking, travel causality, performance and editorial intent
    // This layer is deterministic and preserves the existing shot count/indexes.
    ...
    // Hard-control may refine blocking/pose metadata. Refresh the canonical director state
    // afterwards so image/video prompts inherit the final spatial state.
    episodeScript = directorialOrchestrator.applyToScript(episodeScript, {
      seasonNumber: currentSeason,
      episodeNumber: currentEpisode,
    });
    ...
    // Final reconciliation: hard-control may refine blocking/pose fields. Rebuild
    // canonical director state once more without changing shot count/indexes.
    episodeScript = directorialOrchestrator.applyToScript(episodeScript, {
      seasonNumber: currentSeason,
      episodeNumber: currentEpisode,
    });
  ...
  console.log(`[Pipeline] Temporal contract: provider=${config.videoProvider} min=${_videoTemporalContract().minSeconds}s target=${_videoTemporalContract().targetSeconds}s max=${_videoTemporalContract().maxSeconds}s`);
  ...
  for (let i = 0; i < allShots.length; i++) {
    ...
    if (shotRow?.status === 'mh_submitted' && shotRow.mh_job_id) {
      ...
      try {
        const videoUrl = _normalizeVideoUrl(await videoGen.pollVideoJob(shotRow.mh_job_id, shotRow.mh_api_key), `resumed S${shot.scene_number}/idx${shot.shot_index} poll`);
...
function _selectContinuityCharacterReferences(charsInShot = [], shot = {}, speakerName = '', maxReferences = 3) {
  const chars = Array.isArray(charsInShot) ? charsInShot : [];
  if (!chars.length) return [];

  // Speaker/lead first, then the remaining visible characters in authored order.
  const normalizedSpeaker = String(speakerName || '').trim().toLowerCase();
  const ordered = [
    ...chars.filter(c => normalizedSpeaker && String(c?.name || '').trim().toLowerCase() === normalizedSpeaker),
    ...chars.filter(c => !(normalizedSpeaker && String(c?.name || '').trim().toLowerCase() === normalizedSpeaker)),
  ];

  const staging = shotStaging.getShotCharacterStaging(shot, ordered);
  const selected = selectReferenceAngles({
    characters: ordered,
  ...
  // Keep reference order deterministic and aligned to the selected character rows.
  const result = [];
  for (const item of selected) {
    const character = ordered.find(c => String(c?.name || '').trim().toLowerCase() === String(item?.name || '').trim().toLowerCase());
    const url = typeof item?.url === 'string' ? item.url.trim() : '';
    if (!character || !url) continue;
    if (result.some(x => String(x.character?.name || '').toLowerCase() === String(character.name || '').toLowerCase())) continue;
    result.push({ character, url, angle: item.angle, semantic_reason: item.semantic_reason });
    if (result.length >= maxReferences) break;
  }
...
function _buildLtxVideoPrompt(shot, storyline, orderedChars, positions, motionParams) {
  ...
  // Never hard-truncate authored cinematic prompts. Truncation can amputate
  // dialogue, reaction beats, camera progression, or the outgoing continuity state.
  return finalPrompt;
}
// ──────────────────────────────────────────────────────────────────────────────
  ...
  if (!shot) {
    if (row) {
      return {
        ...
        promptSource: row.last_prompt ? 'submitted_to_video_provider' : (row.video_prompt_override ? 'manual_override' : 'unresolved'),
