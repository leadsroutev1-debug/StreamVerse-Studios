...
function _applyDeterministicSpeakerResolution(dialogueBeats, {
  ...
  const resolved = dialogueBeats.map(beat => {
...
function _buildInitialUser({
  ...
  return [
    ...
    'Describe concrete observable changes in real time: physical action, expression, gaze, gesture, posture, camera movement, environmental change, lighting, sound, spoken performance and terminal visual state.',
    'After establishing the opening frame, explicitly animate EVERY visible person in the image. Enumerate them mentally from the pixels, then give each one an individual chronological motion/reaction beat, including the principal subject, silent listeners, secondary subjects and background people.',
    'Do not let non-speaking characters remain frozen. Give each visible non-speaker a believable independent micro-performance appropriate to their role: breathing, blinking, eye tracking, head turns, posture shifts, weight transfer, hand/finger movement, clothing/hair response, prop interaction, listening reactions or purposeful background activity.',
    'Use motion hierarchy: primary characters receive the strongest story-relevant movement; secondary characters receive smaller but readable reactions; distant background people receive restrained ambient movement. Avoid synchronized or identical motions unless the story explicitly requires coordination.',
    'If the current still contains a person who is not represented in LOCKED CHARACTER HINTS or characters_in_shot, detect them visually anyway and animate them based on the pixels and scene context. Never ignore a visible person because metadata omitted them.',
    'Before returning the final description, verify that every visible person has at least one concrete motion or reaction beat and that the prompt does not describe any visible human as an unchanging statue without an explicit narrative reason.',
...
async function describeForLTX({
  ...
  const system = [
    ...
    'MULTI-CHARACTER MOTION IS MANDATORY: inspect the actual pixels and identify EVERY visible human character in the supplied current still, including primary subjects, secondary subjects, background people, seated people, partially visible people and silent listeners. Do not limit animation to the named speaker or the first character mentioned in metadata.',
    'For EVERY detected visible character, create a distinct motion beat in the chronological prompt. State what that person is doing at the start, how their body/pose changes over time, how their gaze or head position changes, what their hands/arms/shoulders are doing, and how they settle into a believable terminal state.',
    'NO STATIC HUMAN RULE: a visible human must not remain visually frozen for the entire shot unless the image itself makes that character physically immobile for a compelling story reason. Even background or silent characters should receive subtle natural motion such as breathing, blinking, weight shift, eye movement, head turns, hand adjustment, clothing movement, posture correction, environmental interaction or a small purposeful reaction.',
    'SECONDARY CHARACTER AUTONOMY: do not make every character perform the same motion, mirror the lead, or move in synchronized lockstep. Give each detected character an independent, context-appropriate micro-action that fits their role, position, attention and emotional state.',
    'BACKGROUND MOTION PRIORITY: background characters still contribute life to the shot. Keep their movement smaller and less attention-grabbing than the principal action, but explicitly animate them so the frame feels inhabited rather than like a still photograph.',
    'CHARACTER-COUNT CHECK: before writing the final description, mentally count all visible people in the current still and ensure the prompt contains a motion/reaction description for each one. If a person is visible but not named in metadata, describe them from the image instead of ignoring them.',
    'CONTINUITY OF MOTION: when a previous end frame is supplied, compare each visible character across IMAGE 1 and IMAGE 2 and preserve any physically necessary pose, orientation, gaze, hand/prop contact and spatial changes during the transition. Do not reset a secondary character to a static pose simply because they are not the main subject.',
