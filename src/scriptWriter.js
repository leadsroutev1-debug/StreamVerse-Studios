 ...
 */
const PACING_RULES = `
═══ SEMANTIC 10-SECOND CINEMATIC SHOT RULES (mandatory) ═══

You are directing for LTX-2.3 with a HARD MAXIMUM OF 10 SECONDS PER SHOT. Render time is the limiting resource, not per-shot credits, so do not waste the available temporal canvas on underdeveloped fragments. A shot should normally occupy 8–10 seconds unless a provider constraint explicitly prevents it.
...
DURATION INPUTS (the pipeline computes the final duration):
  ...
  6. The resulting LTX duration will normally be 8.0–10.0 seconds and can reach the full 10.0 seconds for rich narrative beats.
...
FORBIDDEN:
  - Do not set duration to 4 or 5 as a fake fallback. Use 10 as the raw intended duration because the backend is LTX-first and the pipeline will clamp safely.
...
async function _ensureSpeechForScene(scene, { storyline, characters }) {
  ...
  // Establishing scenes without any declared character cannot truthfully add a
  // character thought. Leave those intentionally ambient rather than inventing a person.
  if (!speaker) {
...
Prefer a meaningful spoken line when the scene naturally supports an exchange. Otherwise use the character's
internal voice-over thought. Do not invent a new plot event, exposition dump, greeting, or generic filler.
The line must emerge from the immediate emotional beat and what the character knows in this moment.
Return JSON ONLY:
{
  "tts_mode": "spoken | internal_monologue",
  "speaker": "${speaker}",
  "text": "8-25 words of meaningful words actually heard in the scene",
  "delivery_note": "brief unquoted delivery guidance"
}
For internal_monologue, the thought is unquoted and must be formatted downstream as NAME (V.O.): thought.
For spoken dialogue, the words are quoted downstream. Only spoken dialogue uses quotation marks.
`;

  try {
    const repair = await callLLM(
      `${DIRECTOR_PERSONA}\n\nSPEECH COVERAGE GUARD: Every character-led scene must contain meaningful audible speech.\nChoose speech that is truthful to context; use internal voice-over when spoken dialogue would be unnatural.\nOnly spoken dialogue uses quotation marks. Internal voice-over is unquoted and marked NAME (V.O.).`,
      prompt,
      500,
      { useStream: false }
    );
    ...
    const text = String(repair?.text || '').trim().replace(/^['"“”]+|['"“”]+$/g, '');
    if (!text) throw new Error('Speech guard returned empty text');
    ...
    candidate.dialogue_or_action = mode === 'spoken'
      ? `${speaker}: "${text}"${repair?.delivery_note ? ` ${String(repair.delivery_note).replace(/["“”]/g, '')}` : ''}`
      : `${speaker} (V.O.): ${text}`;

    return scene;
  } catch (err) {
    // Deterministic last-resort fallback: still derive the thought from the actual
    // scene/shot context so a transient repair failure cannot silently remove all
    // audible interiority from a character-led scene.
...
${SHOT_SCHEMA}`;
    ...
    try {
      const result = await callLLM(SHOT_SYSTEM_PROMPT, shotUserPrompt, undefined, { useStream: false, temperature: 0.35 });
      ...
      memory.push(_summarizeSceneForMemory(sceneWithShots));
      ...
      if (typeof checkpoint === 'function') {
        ...
        console.log(`[Pipeline] 💾 Scene-shot checkpoint persisted: S${sceneWithShots[0]?.season_number || ''} scene ${scene.scene_number}/${totalScenes}`);
      }
    } catch (err) {
      // Do not silently replace an LLM failure with permanent fallback content.
      // The durable checkpoint keeps prior completed scenes; this scene will retry on resume.
      console.error(`[ScriptWriter] Scene ${scene.scene_number} shot generation failed; preserving earlier checkpoints: ${err.message}`);
      throw err;
    }
  }
