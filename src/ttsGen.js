...
function extractDialogueText(dialogueOrAction) {
  ...
  if (isVoiceOver) {
    ...
    return thought.length > 500 ? thought.slice(0, 500).replace(/\s\S*$/, '').trim() : thought;
  }
...
function extractStrictSpokenDialogue(dialogueOrAction, options = {}) {
  ...
  for (const line of lines) {
    ...
    m = line.match(/^\s*([^:]{1,80}\(PHONE\))\s*:\s*["“]([^"”]+)["”](?:\s+.*)?$/i);
    if (m) {
    ...
    // Internal voice-over is deliberately unquoted.
